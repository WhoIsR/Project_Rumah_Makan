<x-app-layout>
    {{-- Style tambahan untuk tabel responsif --}}
    <style>
        ...
        @media (max-width: 767px) {
            ...
            .responsive-table tbody, .responsive-table tr, .responsive-table td {
                display: block; /* Ubah elemen tabel menjadi block agar bisa ditumpuk */
                width: 100%;
            }
            .responsive-table tr {
                margin-bottom: 1.5rem; /* Beri jarak antar "kartu" bahan */
                border: 1px solid #e5e7eb;
                border-radius: 0.5rem;
                padding: 1rem;
            }
            .responsive-table td {
                padding-left: 50%; /* Buat ruang di kiri untuk label */
                position: relative;
            ...
            <div class="bg-white overflow-hidden shadow-xl rounded-lg">
                ...
                <div class="px-6 py-4 bg-white border-b border-gray-200 flex flex-col md:flex-row md:items-center md:justify-between space-y-2 md:space-y-0">
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                        {{ __('Edit Transaksi Pembelian: ') . $purchase->invoice_number }}
                    </h2>
                </div>
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('admin.purchases.update', $purchase) }}" method="POST" id="purchase-form">
                        ...
                        <input type="hidden" name="supplier_id" id="supplier_id_hidden" value="{{ $purchase->supplier_id }}">

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label for="supplier_location" class="block font-medium text-sm text-gray-700">Lokasi Supplier</label>
                                <select id="supplier_location" class="p-2 mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                    <option value="">-- Pilih Lokasi --</option>
                                </select>
                            </div>
                            <div>
                                <label for="supplier_contact" class="block font-medium text-sm text-gray-700">Nama Pedagang</label>
                                <select id="supplier_contact" class="p-2 mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            ...
                            <div>
                                ...
                                <input type="date" name="transaction_date" id="transaction_date" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('transaction_date', \Carbon\Carbon::parse($purchase->transaction_date)->format('Y-m-d')) }}" required>
                            </div>
                             <div>
                                <label for="invoice_number" class="block font-medium text-sm text-gray-700">Nomor Faktur</label>
                                <input type="text" name="invoice_number" id="invoice_number" value="{{ old('invoice_number', $purchase->invoice_number) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-100" readonly>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4">
                            ...
                            <button type="button" id="open-ingredient-modal-btn" class="mt-2 sm:mt-0 w-full sm:w-auto bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg transition-colors text-center">
                                + Bahan Baru
                            </button>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full responsive-table" id="ingredients-table">
                                <thead>
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style="width: 35%;">Bahan Baku</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style="width: 25%;">Jumlah</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style="width: 20%;">Harga Satuan</th>
                                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider" style="width: 15%;">Subtotal</th>
                                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style="width: 5%;">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody class="md:divide-y md:divide-gray-200">
                                    {{-- Baris akan diisi oleh JavaScript berdasarkan data yang ada --}}
                                </tbody>
                            </table>
                        </div>
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4">
                            <button type="button" id="add-ingredient-btn" class="bg-green-500 hover:bg-green-600 sm:w-auto text-white font-bold py-2 px-4 rounded-lg transition-colors text-center">
                                + Tambah Baris
                            </button>
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="notes" id="notes" rows="3" class="p-3 mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('notes', $purchase->notes) }}</textarea>
                        </div>

                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('admin.purchases.index') }}" class="bg-gray-800 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg transition-colors text-center">Batal</a>
                            <button type="submit" class="ml-4 bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-lg transition-colors text-center">
                                Update Transaksi
                            </button>
                        </div>
                    </form>
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                ...
                <div class="mt-2 px-7 py-3">
                    <form id="new-ingredient-form" onsubmit="return false;">
                        <div class="mb-4 text-left">
                            ...
                            <input type="text" id="new_ingredient_name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                        </div>
                        <div class="mb-4 text-left">
                            <label for="new_ingredient_unit" class="text-sm text-gray-600">Satuan Dasar</label>
                            <select id="new_ingredient_unit" class="p-2 mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                ...
                <div class="items-center px-4 py-3">
                    <button id="save-new-ingredient-btn" class="px-4 py-2 bg-blue-500 text-white text-base font-medium rounded-md w-full shadow-sm hover:bg-blue-600">
                        Simpan
                    </button>
                    <button id="close-ingredient-modal-btn" class="mt-2 px-4 py-2 bg-gray-200 text-gray-800 text-base font-medium rounded-md w-full shadow-sm hover:bg-gray-300">
                        Batal
                    </button>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function populateContactDropdown(selectedLocation) {
    ...
    function selectInitialSupplier() {
        const selectedSupplierId = parseInt(hiddenSupplierIdInput.value);
        const selectedSupplier = allSuppliers.find(s => s.id === selectedSupplierId);

        if (selectedSupplier) {
            supplierLocationDropdown.value = selectedSupplier.name;
            populateContactDropdown(selectedSupplier.name);
            supplierContactDropdown.value = selectedSupplier.id;
        }
    }

    supplierLocationDropdown.addEventListener('change', function() {
        populateContactDropdown(this.value);
        supplierContactDropdown.value = ""; 
    ...
    const addIngredientRow = (detail = null) => {
        ...
        newRow.innerHTML = `
            <td class="px-6 py-4" data-label="Bahan Baku">
            ...
            <td class="px-6 py-4" data-label="Jumlah">
            ...
            <td class="px-6 py-4" data-label="Harga Satuan">
            ...
            <td class="px-6 py-4 subtotal text-right text-sm font-medium" data-label="Subtotal">Rp 0</td>
            <td class="px-6 py-4 action-cell" data-label="">
    ...
    saveNewIngredientBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch("{{ route('admin.ingredients.store.ajax') }}", {
                ...
                body: JSON.stringify({ name, base_unit_id })
            });
            ...
            document.querySelectorAll('.ingredient-select').forEach(select => {
                ...
                newOption.textContent = `${newIngredient.name} (${newIngredient.base_unit.symbol})`;
    ...
    existingDetails.forEach(detail => addIngredientRow(detail));
