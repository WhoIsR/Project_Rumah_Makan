    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h3 class="text-xl font-semibold mb-6 text-center">{{ __('Ringkasan Penjualan') }}</h3>

                {{-- Filter Tanggal --}}
                <div class="mb-6 flex flex-col md:flex-row items-end md:items-center space-y-4 md:space-y-0 md:space-x-4 bg-gray-50 p-4 rounded-lg">
                    <form action="{{ route('admin.reports.index') }}" method="GET" class="flex flex-col md:flex-row items-end md:items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
                        <div>
                        ...
                        <div>
                            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold mt-6 py-2 px-4 rounded-lg transition-colors">
                                Filter
                            </button>
                        </div>
                        <div class="flex-grow"></div> {{-- Spacer --}}
                        <div>
                            <a href="{{ route('admin.reports.daily', ['date' => $startDate]) }}"
                ...
                <h3 class="text-xl font-semibold mb-4 border-t pt-6">{{ __('5 Menu Terlaris') }}</h3>
                ...
                {{-- Catatan: Visualisasi data (chart) bisa ditambahkan di sini menggunakan Chart.js atau library lain --}}
