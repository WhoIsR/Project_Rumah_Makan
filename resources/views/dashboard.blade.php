    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <!-- Kartu Total Menu -->
                <a href="{{ route('admin.menu-items.index') }}" class="bg-gradient-to-br from-blue-500 to-indigo-600 text-white p-6 rounded-xl shadow-lg hover:scale-105 transform transition-transform duration-300">
                    <div class="flex justify-between items-start">
                        ...
                        <div class="p-3 bg-white/30 rounded-full">
                             <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7"></path></svg>
                        </div>
                    </div>
                </a>

                <!-- Kartu Total Bahan -->
                <a href="{{ route('admin.ingredients.index') }}" class="bg-gradient-to-br from-green-500 to-emerald-600 text-white p-6 rounded-xl shadow-lg hover:scale-105 transform transition-transform duration-300">
                     <div class="flex justify-between items-start">
                        <div class="flex-grow">
                ...
                <!-- Jalan Pintas: Tambah Menu -->
                 <a href="{{ route('admin.menu-items.create') }}" class="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl hover:bg-gray-50 transform transition-all duration-300 flex flex-col justify-center items-center text-center">
                ...
                <a href="{{ route('admin.ingredients.create') }}" class="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl hover:bg-gray-50 transform transition-all duration-300 flex flex-col justify-center items-center text-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-xl font-semibold mb-4 border-b pb-3 text-gray-800">🍽️ Menu Baru Ditambahkan</h3>
                        <ul class="space-y-4 mt-4">
                            @forelse ($recentMenus as $menu)
                                <li class="flex items-center space-x-4">
                                    <img src="{{ $menu->image_path ? Storage::url($menu->image_path) : 'https://placehold.co/64x64/e2e8f0/64748b?text=...' }}" alt="{{ $menu->name }}" class="w-16 h-16 object-cover rounded-lg shadow-md flex-shrink-0">
                                    <div class="flex-grow">
                                        <p class="font-bold text-gray-900">{{ $menu->name }}</p>
                                        <p class="text-sm text-gray-600">Rp {{ number_format($menu->price, 0, ',', '.') }}</p>
                                    </div>
                                    <a href="{{ route('admin.menu-items.edit', $menu) }}" class="text-sm text-indigo-600 hover:text-indigo-900 font-semibold">Edit</a>
                                </li>
                            @empty
                    ...
                    <div class="p-6">
                        ...
                        <ul class="space-y-3 mt-4">
                             @forelse ($lowStockIngredients as $ingredient)
                                <li class="flex justify-between items-center p-3 rounded-lg {{ $ingredient->stock <= 10 ? 'bg-red-50' : 'bg-yellow-50' }}">
                                    <span class="font-medium text-gray-700">{{ $ingredient->name }}</span>
                                    <div class="text-right">
                                        <p class="font-bold {{ $ingredient->stock <= 10 ? 'text-red-600' : 'text-yellow-600' }}">
